...
For a production deployment behind a reverse proxy (our Dokploy setup), see
[Production deployment behind a reverse proxy (Dokploy)](#production-deployment-behind-a-reverse-proxy-dokploy)
below.
...
### MCP over HTTP with bearer auth (Dokploy)

For a remote deployment behind our Dokploy reverse proxy, use the streamable
HTTP endpoint with a bearer token (see
[Production deployment behind a reverse proxy (Dokploy)](#production-deployment-behind-a-reverse-proxy-dokploy)):
...
`streamable-http` is accepted as an alias for `http`. The `http`/`streamable-http`
and `sse` transports are unauthenticated by default — anyone who can reach
`MCP_HOST`:`MCP_PORT` can call tools. To expose the server on a public domain,
also set `MCP_BEARER_TOKEN`, which requires an `Authorization: Bearer <token>`
header on every request (except `/health`); see
[Production deployment behind a reverse proxy (Dokploy)](#production-deployment-behind-a-reverse-proxy-dokploy).
...
### Production deployment behind a reverse proxy (Dokploy)

Our Dokploy deployment runs the shared HTTP server on a public domain, so it
adds bearer-token auth on top of the plain `http` transport described above:

- `MCP_TRANSPORT=streamable-http` (alias for `http`)
- `MCP_HOST=0.0.0.0`, `MCP_PORT=8000`, `MCP_PATH=/mcp`
- `MCP_BEARER_TOKEN` — required; every request except `/health` must send
  `Authorization: Bearer <MCP_BEARER_TOKEN>`
- `MCP_ALLOWED_HOSTS` — comma-separated hostnames for DNS-rebinding protection

With those defaults, MCP is served at `http://localhost:8000/mcp` and health
checks at `http://localhost:8000/health`. If the bearer token is missing or
stale, the server returns a machine-readable `401`, e.g.:
...
For internet-facing deployments, put Cloudflare Access in front of the MCP
hostname (DNS record proxied through Cloudflare, Access application using a
Service Auth policy). Requests then need two independent auth layers:
...
- Cloudflare Access rejects missing or expired Cloudflare service credentials
  before traffic reaches the MCP server, returning `403` from Cloudflare.
...
#### Multiple Telegram accounts, multiple Dokploy apps

Run one Dokploy application per Telegram account. Each app uses the same
repository, branch, and Dockerfile, but has its own `TELEGRAM_SESSION_STRING`,
`MCP_BEARER_TOKEN`, `TELEGRAM_ACCOUNT_NAME`, and domain.
